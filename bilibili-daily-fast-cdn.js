...
(function () {
  ...
  function handlePlayUrlResponse() {
    ...
    if (candidates.length < 2) {
      clearLock();
      writeDailyState({ day: day, status: "no-candidates", mode: "playurl", updatedAt: Date.now() });
      finish({});
      return;
    }
    ...
    benchmark(candidates, requestHeaders(), function (results) {
      ...
      if (!winner) {
        writeDailyState({ day: day, status: "no-winner", mode: "playurl", updatedAt: Date.now() });
        finish({});
        return;
      }
      ...
      if (!winnerInfo) {
        writeDailyState({ day: day, status: "invalid-winner", mode: "playurl", updatedAt: Date.now() });
        finish({});
        return;
      }
  ...
  function handleMediaRequest() {
    ...
    if (candidates.length < 2) {
      clearLock();
      writeDailyState({ day: day, status: "no-candidates", mode: "media", updatedAt: Date.now() });
      finish({});
      return;
    }
    ...
    benchmark(candidates, requestHeaders(), function (results) {
      ...
      var winner = selectWinner(results);
      if (!winner) {
        writeDailyState({ day: day, status: "no-winner", mode: "media", updatedAt: Date.now() });
        finish({});
        return;
      }
      ...
      var winnerInfo = parseUrl(winner.url);
      if (!winnerInfo) {
        writeDailyState({ day: day, status: "invalid-winner", mode: "media", updatedAt: Date.now() });
        finish({});
        return;
      }
  ...
  function readDailyState(day) {
    var state = readStoredJson(STORE_KEY);
    return state && state.day === day ? state : null;
  }
  ...
  function acquireLock(day, mode) {
    var lock = readStoredJson(LOCK_KEY);
    var now = Date.now();
    if (lock && lock.day === day && typeof lock.startedAt === "number" && now - lock.startedAt < config.lockMilliseconds) {
      return false;
    }
    if (typeof $persistentStore !== "undefined" && $persistentStore && typeof $persistentStore.write === "function") {
      return $persistentStore.write(JSON.stringify({ day: day, mode: mode, startedAt: now }), LOCK_KEY) !== false;
    }
    return false;
  }
